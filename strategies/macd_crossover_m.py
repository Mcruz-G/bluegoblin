...
def apply_techincals(data, macd_periods):
    ...
    data[["MACD", "S_MACD"]] = ta.macd(data["CLOSE"], fast=fast_macd_period, slow=slow_macd_period, signal= 9*60).loc[:,[f"MACD_{fast_macd_period}_{slow_macd_period}_{540}", f"MACDs_{fast_macd_period}_{slow_macd_period}_{540}"]]
    data["METRIC"] = (abs(data["MACD"]) - abs(data["S_MACD"])) / (abs(data["MACD"]) + abs(data["S_MACD"]))
...
def apply_rule(data, macd_periods):
    ...
    data["RULE"] = np.logical_and(data["RULE"], data["METRIC"] < 0.10) 
